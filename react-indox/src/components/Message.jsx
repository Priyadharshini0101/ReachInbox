...
useEffect(() => {
  ...
  return (
    <div className='mx-4 hover:bg-gray-200 dark:hover:bg-gray-800 ' onClick={() => openMail()}>  
    <hr className="  dark:border-[#323440] border-transparent "></hr> 
    <div id={id} className={`cursor-pointer flex flex-col px-[8px] py-[12px] gap-[8px] ${  id === currentMessage.threadId ? `border-blue-500 border-l border-l-[2.5px]`:``}` }>
        <div className='flex justify-between'>
        <h1 className='text-[14px] font-medium text-[#343A40] dark:text-white'>{email}</h1>
        <p className='text-[12px] text-[#919EAB] dark:text-gray-500'>{currentDate ? currentDate.slice(0,6):''}</p>
        </div>
        <p className='text-[12px] text-[#343A40] dark:text-[#E1E0E0]'>{parse(text.slice(0,35))}</p>
